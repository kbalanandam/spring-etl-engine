...
The goal is to support common audit and operational field defaults such as `createdUser`, `updatedUser`, `createdDate`, and `updatedDate` without forcing every job bundle to repeat the same field-by-field expression logic.
...
flowchart TD
    ...
    B --> C[Normal fields[] mapping from source]
    B --> D[Mapping-level defaultValues[] generated by runtime]
...
- gives the product a natural home for standard tokens such as job name, current timestamp, and constant values
