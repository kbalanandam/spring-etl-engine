...
| `com.etl.processor` factory | `DynamicProcessorFactory`, `DynamicProcessor` | `REUSE` | Shared processor dispatch remains a valid extension seam after S6 cutover to one active `type: default` contract. | Keep as the default-processor dispatch seam; extend behavior through processor rules/transforms instead of alternate processor types. |
