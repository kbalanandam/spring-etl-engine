...
- [`S1 - Define schedule model and trigger contract for scenario-based execution`](../backlog-items/scheduler/S1-schedule-model-and-trigger-contract.md)
- [`S2 - Add time-based schedule definitions with pause/resume controls`](../backlog-items/scheduler/S2-time-based-schedule-definitions-with-pause-resume.md)
- [`S3 - Add overlap policy, missed-run handling, and basic trigger audit trail`](../backlog-items/scheduler/S3-overlap-policy-missed-run-handling-and-trigger-audit-trail.md)
- [`S4 - Define control-plane operational data model for schedules, watchers, trigger events, run and step history, artifact lineage, and restartability anchors`](../backlog-items/scheduler/S4-control-plane-operational-data-model.md)
