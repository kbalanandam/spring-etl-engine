...
| [A7b](backlog-items/etl-core/A7b-custom-step-context-outcome-and-failure-finalization-follow-on.md) | Extend custom-step context bridge, outcome mapping, and failure-finalization contract | Epic A | P1 | Done | M2 | A7, D1 | Completed with typed context handoff checks, deterministic write-once context ownership, explicit `CONTINUE/STOP/FAIL` mapping, and bounded failure-finalizer hooks |
...
| [R2](backlog-items/scheduler/R2-multi-rdbms-datasource-and-dialect-profile-contract.md) | Define multi-RDBMS datasource and dialect profile contract | Epic R | P1 | In Progress | M3 | R1 | Validation evidence captured (`target/tmp-r2-targeted-tests.log`, `target/tmp-r2-profile-tests.log`, `target/tmp-r2-verify-recent.log`, `target/tmp-r2-guard-tests.log`, `target/tmp-r2-next-task-tests.log`); remaining gate is explicit direct selected-job ETL proof when control-plane persistence is unavailable |
...
3. Make `R2` the immediate gate: close the remaining explicit direct selected-job ETL proof for control-plane-persistence-unavailable conditions before `R3` expands persistence internals.
...
- [x] Define the retained control-plane operational data model for schedules, watchers, trigger events, run / step history, artifact lineage, and restartability anchors
