...
| [S4](backlog-items/scheduler/S4-control-plane-operational-data-model.md) | Define control-plane operational data model for schedules, watchers, trigger events, run and step history, artifact lineage, and restartability anchors | Epic S | P1 | In Progress | M2 | S1, C1, C2 | Current-release S4a slice is shipped (schedule/trigger/run PK-cutover foundations); next S4b chunk targets durable `step_record` + `artifact_record` persistence with explicit artifact-ownership invariants (tracked in [`S4b checklist`](backlog-items/scheduler/S4b-step-artifact-persistence-checklist.md)), while `attempt_link`/`checkpoint_anchor` stay later with `F1` |
