...
| [U2](backlog-items/operator-ui/U2-run-detail-drilldown-with-step-and-artifact-evidence.md) | Add job run detail drill-down with step outcomes, evidence links, and run-scoped log viewer | Epic U | P1 | In Progress | M2 | U1, C2 | Reopened U2 to extend run-detail UX from plain full-file log links to richer in-page rendering and run-instance-scoped log content (without widening scheduler/launch boundaries) |
