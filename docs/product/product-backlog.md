...
| [S4](backlog-items/scheduler/S4-control-plane-operational-data-model.md) | Define control-plane operational data model for schedules, watchers, trigger events, run and step history, artifact lineage, and restartability anchors | Epic S | P1 | In Progress | M2 | S1, C1, C2 | Current-release S4a slice is shipped (schedule/trigger/run PK-cutover foundations); S4b baseline is now complete with durable `step_record` + `artifact_record` schema, lineage guardrails, step+artifact projection writes, persisted read-model APIs, and Operator UI run-detail consumption (tracked in [`S4b checklist`](backlog-items/scheduler/S4b-step-artifact-persistence-checklist.md)); next retained-history expansion is `S4c` (`attempt_link`/`checkpoint_anchor`) with `F1` |
...
12. Run `S1` contract freeze as the active scheduler boundary and execute first `S2` + phased `S4` slices in the same release lane (`S4a` shipped; `S4b` next for `step_record`/`artifact_record`), while keeping advanced `S3` overlap/missed-run policy deferred.
