...
| [U2](backlog-items/operator-ui/U2-run-detail-drilldown-with-step-and-artifact-evidence.md) | Add job run detail drill-down with step outcomes, evidence links, and run-scoped log viewer | Epic U | P1 | In Progress | M2 | U1, C2 | Reopened U2 to extend monitoring UX with runs-list job scoping, per-run drill-down from that selected-job context, and richer run-instance-scoped in-page log rendering (without widening scheduler/launch boundaries) |
