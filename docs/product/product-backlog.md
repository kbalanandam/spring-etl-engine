...
3. Make `R2` the immediate gate: close the remaining explicit valid/invalid mode+vendor matrix proofs before `R3` expands persistence internals (fallback proof, profile-token consistency, and runbook updates are now documented with evidence).
