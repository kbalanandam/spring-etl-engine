...
Define one shared naming contract for OneFlow-generated model packages and class names so explicit job bundles can stop depending on authored `packageName` values while runtime lookup, XML generation, and processor mapping all stay deterministic and validated.
...
- class names do not clearly identify that they were generated by OneFlow or whether they represent an ingress source, an intermediate handoff, or a final target
...
- step-aware runtime role derivation for `Source`, `Intermediate`, and `Target`
...
3. derive an additional runtime role from ordered `steps[]` reuse so generated class names can distinguish `Source`, `Intermediate`, and `Target`
4. standardize generated class names as `OneFlowGenerated<NormalizedConfigName><FlowRole><Shape>`
5. keep XML Java type names separate from XML contract names by using JAXB annotations for `rootElement` and `recordElement`
6. treat authored `packageName` as a temporary bridge only: matching values warn, conflicting values fail
7. update docs, preserved bundles, and focused tests in the same slice so the contract is executable and reviewable
...
- [ ] one documented naming policy derives generated class names from logical config names plus runtime role and shape
...
- [ ] validation fails fast for logical-name collisions, invalid intermediate reuse, forward-reference mistakes, and conflicting legacy `packageName` values
...
The multi-step TVL example in `private-jobs/TVL/xml-nested-to-csv-tag-validation/` is a useful design proof because it contains an ingress XML source, an intermediate CSV handoff, and a final relational target in one selected job.
