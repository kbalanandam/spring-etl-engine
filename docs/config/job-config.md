...
| `recoveryPolicy` | no | string | Optional selected-run restart policy evidence (`rerun-from-start` default). `resume-from-checkpoint` is recognized but currently fails fast as unsupported in the shipped runtime |
...
- `recoveryPolicy` is optional; omit it for default `rerun-from-start`, or set it explicitly for operator-evidence clarity. `resume-from-checkpoint` currently fails fast until a later F1 implementation slice ships checkpoint resume runtime behavior.
...
- The optional top-level `recoveryPolicy` defaults to `rerun-from-start`; `resume-from-checkpoint` is currently a guarded future token and fails fast as unsupported.
