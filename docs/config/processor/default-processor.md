...
- Unsupported `transforms[].type` and `rules[].type` now fail fast with operator-facing context that includes scenario name, `processor-config.yaml` path, source-target entity mapping, field mapping, and source format.
