...
DROP PROCEDURE IF EXISTS upsert_pk_sequence_floor;
...
CREATE PROCEDURE upsert_pk_sequence_floor(
    IN p_sequence_name VARCHAR(120),
    IN p_next_value BIGINT
)
BEGIN
    INSERT INTO controlplane_pk_sequence (
        sequence_name,
        next_value,
        updated_at,
        created_by,
        ...
        p_sequence_name,
        GREATEST(IFNULL(p_next_value, 1), 1),
        CURRENT_TIMESTAMP,
        'spring-etl-engine-bootstrap',
        'spring-etl-engine-bootstrap'
    )
    ON DUPLICATE KEY UPDATE
        next_value = GREATEST(next_value, VALUES(next_value)),
        updated_at = CURRENT_TIMESTAMP,
        updated_by = 'spring-etl-engine-bootstrap';
END$$
DELIMITER ;
...
CALL drop_index_if_exists('controlplane_trigger_event', 'idx_trigger_event_schedule_time');
...
CALL add_index_if_missing('controlplane_run_summary', 'idx_run_summary_pk', 'run_summary_pk', TRUE);
CALL add_index_if_missing('controlplane_run_summary', 'idx_run_summary_run_record_pk', 'run_record_pk', FALSE);
...
CALL upsert_pk_sequence_floor('controlplane_schedule_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_trigger_event_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_run_summary_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_run_record_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_step_record_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_artifact_record_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_attempt_link_pk', 1);
CALL upsert_pk_sequence_floor('controlplane_checkpoint_anchor_pk', 1);
