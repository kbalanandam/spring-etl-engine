...
- On the explicit-job path, source/target `packageName` is no longer part of the active contract. Omit it in new or updated bundles so `JobScopedPackageNameResolver` derives `com.etl.generated.job.<normalized-job-name>.source|target` from `job-config.yaml -> name`; explicit selected-job runs now fail fast if authored `packageName` is present (`src/main/java/com/etl/common/util/JobScopedPackageNameResolver.java`, `docs/architecture/generated-model-naming-standard.md`).
